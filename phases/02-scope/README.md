...
- [x] Conflict Alert uses kinematic CPA and the lower pair airspace type (Type 1–4); unacknowledged CA flashes red `CA` on Line 0 above the ACID at 800 ms on / 800 ms off, then stays solid red when acknowledged. Concurrent MSAW/CA is `LA/CA`. Field 2 inhibits beside the ACID are upright `Δ` for CA/MCI, `*` for MSAW, and `+` for both; LA/CA/MCI list rows stay green and never flash. `CA K`, `CA`, `CA P`, and `CA E` accept typed ACIDs or slew selection and replace the rejected `*CA` alias; `kdem-ca` is the manual bench. Still **no** 3 NM CA halo; circles on this scope are TPA J-rings only.
