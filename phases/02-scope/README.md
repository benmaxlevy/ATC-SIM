...
- [x] Conflict Alert uses kinematic CPA and the lower pair airspace type (Type 1–4); active CA/MSAW/MCI paint a red inline `+` beside the ACID, blinking 800 ms on / 800 ms off until CA/MSAW acknowledge. CA/MCI inhibit is normal upright `Δ`; MSAW inhibit is `*`. `CA K`, `CA`, `CA P`, and `CA E` accept typed ACIDs or slew selection and replace the rejected `*CA` alias; `kdem-ca` is the manual bench. Still **no** 3 NM CA halo; circles on this scope are TPA J-rings only.
