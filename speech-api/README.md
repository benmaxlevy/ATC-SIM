...
**Roster + catalog grounding (not a vector DB):** the sim may send `context.callsigns` as structured canonical candidates (`{ callsign, aliases }`), plus `context.selectedCallsign`, `context.fixes` (facility fix ids), `context.procedures`, `context.approaches`, and separate `context.airports` (ICAO/name/aliases for IFR clearance limits). Those go in the **user** turn as `onFrequency=` / `aircraftCandidates=` / `fixes=` / `procedures=` / `approaches=` / `airports=` so the static system prompt stays cacheable. A complete authored alias plus registration tail (e.g. `Skyhawk 123`) may ground only to its one listed canonical `N123`; alias-only, unknown, incomplete, or ambiguous evidence is a soft `PARSE_MISS`, and `callsignToken` never contains alias text. Candidate aliases are bounded authored evidence, not a free-form search list; the complete tail is one through five digits and any suffix letters remain part of the canonical registration. The GBNF and semantic validator accept only the listed canonical callsign output. This is local simulator salvage, not FAA-complete natural-language understanding. The model must pick an ICAO from the roster (e.g. ASR `giblet 204` → `SWA204`) and a listed fix spelling (e.g. ASR `C-Max` → `SEMAX`). Airport candidates may ground only `IFR_CLEARANCE.limitId`, never `DIRECT` or `CROSS`. Do not send kinematics — Path C is not an executor. The browser also snaps a unique flight-number suffix onto the roster, and a unique noisy `fixId` onto the catalog, when the model still returns junk.
