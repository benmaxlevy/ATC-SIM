...
function getBoxValue(strip: ArrivalStripData, boxKey: string): string {
...
export function ArrivalStrip({
  ...
  return (
    <div
      className={`strip arrival-strip ${selected ? "strip-selected" : ""} ${isIndented ? "strip-indented" : ""} ${isDragging ? "strip-dragging" : ""} ${className ?? ""}`.trim()}
      role="button"
      tabIndex={0}
