...
test("SessionEvent includes command events, voice.latency, CA edges, MSAW edges, and nav FMS", () => {
  expectTypeOf<SessionEvent["type"]>().toEqualTypeOf<
    ...
    | "class_b.clearance.issued"
    | "class_b.clearance.accepted"
    | "class_b.clearance.rejected"
    | "class_b.entered"
    | "class_b.exited"
    | "class_b.route_deviation"
    | "class_b.altitude.resumed"
    | "voice.latency"
    | "alert.ca.caution"
    | "alert.ca.alert"
    | "alert.ca.clear"
...
test("radio.checkin is a typed pilot-initiated payload (T04-15)", () => {
  ...
  log.append({
    ...
    starName: "DEMO ONE",
    altitudeFt: 11000,
    text: "Approach, Delta 123, descending via DEMO ONE arrival through one-one thousand (11000)",
  });
