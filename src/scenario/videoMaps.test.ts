...
test("AC1 — loadKdem videoMapSet is KDEM with RWY, LOC, COAST plus extras", () => {
  ...
  expect(ids).toEqual(["RWY27", "LOC27", "COAST", "DWNWND", "CLASS_B"]);
});

test("KDEM catalog loads five MAPS in ARP ENU NM", () => {
  const maps = loadVideoMapSet("KDEM");
  expect(maps.map((item) => item.id)).toEqual(["RWY27", "LOC27", "COAST", "DWNWND", "CLASS_B"]);
