...
- [x] Conflict Alert uses kinematic CPA and the lower pair airspace type (Type 1–4); only unacknowledged Line 0 CA/MSAW indicators flash red (`CA`, `LA`, or `LA/CA`) at 800 ms on / 800 ms off, then stay solid after an empty-Preview slew-click acknowledgement. Field 2 inhibits beside the ACID are upright `Δ` for CA/MCI, `*` for MSAW, and `+` for both; LA/CA/MCI list rows stay green and never flash, with CA rows `CA <ACID> * <ACID>`. `CA K`, `CA`, `CA P`, and `CA E` accept typed ACIDs or slew selection and replace the rejected `*CA` alias; `kdem-ca` is the manual bench. Still **no** 3 NM CA halo; circles on this scope are TPA J-rings only.
