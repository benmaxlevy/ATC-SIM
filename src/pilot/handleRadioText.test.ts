...
test("AC1 — DAL123 H270 accepted; assigned 270 SHORTEST; others unchanged; heading template; command.accepted", () => {
...
test("AC3 — APP ILS27 sets clearedApproachId; heading/alt/speed intent unchanged", () => {
  const dal = sample("DAL123", "ac-dal");
  const before = snapshot(dal);
  const world = createWorld({ aircraft: [dal] });
  const log = new SessionLog();

  const result = handleRadioText(world, "DAL123 APP ILS27", log);
  expect(result.accepted).toBe(true);
...
test("AC4 — C30 at 8000 rejected CLIMB_NOT_ABOVE; intent unchanged; command.rejected", () => {
  const dal = sample("DAL123", "ac-dal", { altitudeFt: 8000 });
  const before = snapshot(dal);
  const world = createWorld({ aircraft: [dal] });
  const log = new SessionLog();

  const result = handleRadioText(world, "DAL123 C30", log);
...
test("AC5 — S400 speed reject; H370 parse reject; empty string reject; no intent change", () => {
  const dal = sample("DAL123", "ac-dal");
  const world = createWorld({ aircraft: [dal] });
  const log = new SessionLog();

  const beforeSpeed = snapshot(dal);
  const speed = handleRadioText(world, "DAL123 S400", log);
  ...
  const heading = handleRadioText(world, "DAL123 H370", log);
  ...
  const empty = handleRadioText(world, "", log);
  expect(empty.accepted).toBe(false);
  expect(empty.reason).toBe("PARSE");
  expect(dal.intent).toEqual(beforeEmpty.intent);
});

test("AC6 — ambiguous suffix 123 rejects; both assigned headings unchanged", () => {
  const dal = sample("DAL123", "ac-dal", { headingDeg: 100 });
  ...
  const result = handleRadioText(world, "123 H270", log);
  expect(result.accepted).toBe(false);
...
test("AC7 — D30 assigns 3000; S210 assigns 210; PH snaps assigned heading to current", () => {
  const dal = sample("DAL123", "ac-dal", { altitudeFt: 8000, headingDeg: 10, speedKt: 220 });
  const world = createWorld({ aircraft: [dal] });
  const log = new SessionLog();

  const descend = handleRadioText(world, "DAL123 D30", log);
  expect(descend.accepted).toBe(true);
  expect(dal.intent.assignedAltitudeFt).toBe(3000);

  const speed = handleRadioText(world, "DAL123 S210", log);
  ...
  const ph = handleRadioText(world, "DAL123 PH", log);
...
test("T20L alone turns from present heading", () => {
  const dal = sample("DAL123", "ac-dal", { headingDeg: 10 });
  const world = createWorld({ aircraft: [dal] });
  const result = handleRadioText(world, "DAL123 T20L", new SessionLog());
  expect(result.accepted).toBe(true);
  expect(dal.intent.assignedHeadingDeg).toBe(350);
  expect(dal.intent.turn).toBe("LEFT");
});

test("combined apply is last-wins for heading: H270 then T20L uses present heading", () => {
  const dal = sample("DAL123", "ac-dal", { headingDeg: 10 });
  const world = createWorld({ aircraft: [dal] });
  const result = handleRadioText(world, "DAL123 H270 T20L", new SessionLog());
  expect(result.accepted).toBe(true);
  expect(dal.intent.assignedHeadingDeg).toBe(350);
  expect(dal.intent.turn).toBe("LEFT");
});

test("reject does not partially apply a combined command", () => {
  const dal = sample("DAL123", "ac-dal", { altitudeFt: 8000, headingDeg: 100 });
  const before = snapshot(dal);
  const world = createWorld({ aircraft: [dal] });
  const result = handleRadioText(world, "DAL123 H270 C30", new SessionLog());
  expect(result.accepted).toBe(false);
  expect(result.reason).toBe("CLIMB_NOT_ABOVE");
  expect(dal.intent).toEqual(before.intent);
});

test("AC8 — handleRadioText tests run without window, document, or rAF", () => {
  expect(typeof globalThis.window).toBe("undefined");
  expect(typeof globalThis.document).toBe("undefined");
  expect(typeof globalThis.requestAnimationFrame).toBe("undefined");
});

test("pilot sources do not import @scope, @ui, or stepWorld", () => {
...
test("grep guard: no intent.assigned writes outside src/pilot except tests and createAircraft", () => {
