...
- **Phase folder:** `phases/NN-name/` (one of `00-slice`, `01-closed-loop`, `02-scope`)
- **Max in-flight workers:** 3
- **Waves:** use the wave table in `phases/SWARM.md` for this phase (not the whole-phase `AGENT.md` — that would overload one agent)
...
3. When all tickets in this phase are on `master`, run the phase README **Phase exit** checklist (tests + whatever you can verify without a long manual playtest; Manual ACs: do them if you can run the app, else list them for the human).
