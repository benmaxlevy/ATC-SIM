...
import { DEFAULT_STT_URL, DEFAULT_TTS_URL } from "./http-speech-port";

const DEFAULT_URLS = { sttUrl: DEFAULT_STT_URL, ttsUrl: DEFAULT_TTS_URL };

test("AC1 / AC6 — URLs present including 127.0.0.1:8090 defaults pick http", () => {
...
test("AC2 — URLs explicitly cleared pick null, never web-speech", () => {
...
test("webSpeech flag must not auto-select (never the quality default)", () => {
  expect(pickDefaultBackend({ ...DEFAULT_URLS, webSpeech: true })).toBe("http");
  expect(pickDefaultBackend({ sttUrl: "", ttsUrl: "", webSpeech: true })).toBe("null");
  expect(pickDefaultBackend({ webSpeech: true })).not.toBe("web-speech");
});
...
test("factory returns the requested id; unknown and whisper-wasm become null", () => {
  expect(createSpeechPort("http").id).toBe("http");
  expect(createSpeechPort("web-speech").id).toBe("web-speech");
...
test("resolveSpeechBackend restores opt-in web-speech; http still needs URLs", () => {
  expect(resolveSpeechBackend("web-speech", { sttUrl: "", ttsUrl: "" })).toBe("web-speech");
  expect(resolveSpeechBackend("null", DEFAULT_URLS)).toBe("null");
...
test("createBootSpeechPort never throws; cleared URLs → null; defaults → http", () => {
  expect(createBootSpeechPort(undefined, {}).id).toBe("http");
  expect(createBootSpeechPort(undefined, { VITE_STT_URL: "", VITE_TTS_URL: "" }).id).toBe("null");
  expect(createBootSpeechPort("web-speech", { VITE_STT_URL: "", VITE_TTS_URL: "" }).id).toBe(
    "web-speech",
  );
  expect(createBootSpeechPort("http", { VITE_STT_URL: "", VITE_TTS_URL: "" }).id).toBe("null");
});
