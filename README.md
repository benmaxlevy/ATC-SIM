...
- **On-Scope System Lists Window Manager**: Interactive, draggable operational windows (Flight Plan `FL`/`TAB`, Tower `TL` and satellite towers `*TL<ID>`, VFR `VL`, Video Maps `ML`, Alert Status `AL`, CRDA `CR`, Coast/Suspend `CS`, Sign-On `SO`, and SSA `*S`). Features title-bar drag handles, collision warning outlines, quick default reset (`*[ID] D` / Shift+Click), `F1` row drops, and flight plan correlation.
- **Unified STARS Preview Area**: Dedicated on-scope command buffer beneath the SSA for track initiation (`+`), track drop (`/`), leader formatting (`*L(1-9)`, `*LDR`), forced datablocks (`*F`), flight plan correlation (`[Index#]`), safety alert inhibits (`*CA`, `*LA`, `*MCI`), video maps (`*D`, `MAP`), range rings (`*RR`), weather reflectivity levels (`*WX`), and altitude/beacon filters (`*LA`, `*BCN`, `B##`).
...
- **Safety Logic**: Continuous Conflict Alert (CA), Minimum Safe Altitude Warning (MSAW), and Mode C Intruder (MCI) alerting with visual datablock flashing, audio alarms, and Alert Status box tracking.
...
| ↳ [STARS Preview Area Commands](docs/USER.md#stars-preview-area-commands) | Unified scope buffer reference: tracking (`+`/`/`), leader lines/lengths, forced FDB, flight plan correlation (`[Index#]`), alert inhibits (`*CA`, `*LA`, `*MCI`), video maps, display, and filters. |
| ↳ [System Lists & Window Manager](docs/USER.md#system-lists--window-manager) | Interactive operational lists (`FL`, `TL`, `*TL<ID>`, `VL`, `ML`, `AL`, `CR`, `CS`, `SO`, `SSA`), drag handles, collision boxes, and `F1` row drops. |
