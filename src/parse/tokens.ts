 ...
 * (T04-05). `IL ILS27` is INTERCEPT_LOCALIZER (loc only). `GA` is GO_AROUND
 * (T04-07). Not vice-compatible.
...
export function isTransitionIdToken(token: string): boolean {
  return TRANSITION_ID_TOKEN.test(token);
}
