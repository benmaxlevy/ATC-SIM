...
export type ValidateReason =
  "EMPTY" | "HEADING" | "ALTITUDE" | "SPEED" | "CLIMB_NOT_ABOVE" | "DESCEND_NOT_BELOW";
...
export function validateInstructions(
  ...
  for (const instruction of instructions) {
    const result = validateOne(aircraft, instruction);
    if (!result.ok) {
      return result;
    }
  }
...
function validateOne(aircraft: Aircraft, instruction: Instruction): ValidateResult {
